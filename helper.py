...
def load_data(flag='stage1'):
    print('Loading data...')
    train_variant = pd.read_csv("data/training_variants").fillna("")
    train_text = pd.read_csv("data/training_text", sep="\|\|", engine='python', header=None, skiprows=1, names=["ID", "Text"]).fillna("")
    test_variant = pd.read_csv("data/test_variants").fillna("")
    test_text = pd.read_csv("data/test_text", sep="\|\|", engine='python', header=None, skiprows=1, names=["ID", "Text"]).fillna("")
    if flag == 'stage1':
